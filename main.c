...
#include "bin_tree/BinTree.h"
#include "b/b_tree.h"
#define POISON_VAL_TEST -1
...
void TestTimeAllCtor(TestTimeAll *obj) {
  obj->Avl.Insert = POISON_VAL_TEST;
  obj->Bin.Insert = POISON_VAL_TEST;
  obj->Decart.Insert = POISON_VAL_TEST;
  obj->Splay.Insert = POISON_VAL_TEST;
  obj->Skip.Insert = POISON_VAL_TEST;
  obj->RBT.Insert = POISON_VAL_TEST;
  obj->Splay.Insert = POISON_VAL_TEST;
  obj->Btree.Insert = POISON_VAL_TEST;
  obj->Avl.Remove = POISON_VAL_TEST;
  obj->Bin.Remove = POISON_VAL_TEST;
  obj->Decart.Remove = POISON_VAL_TEST;
  obj->Splay.Remove = POISON_VAL_TEST;
  obj->Skip.Remove = POISON_VAL_TEST;
  obj->RBT.Remove = POISON_VAL_TEST;
  obj->Splay.Remove = POISON_VAL_TEST;
  obj->Btree.Remove = POISON_VAL_TEST;
}
...
TestTimeAll do_test(int size, int max_size) {
  ...
  if ((Avl_elems == NULL) || (Bin_elems == NULL) || (Decart_elems == NULL) || (Splay_elems == NULL) ||
      (RBT_elems == NULL) || (B_tree == NULL) || (skip.arr == NULL) || (data == NULL) || (index == NULL)) {
    return ret;
  }
...
int main() {
  ...
  for (int i = 1; i < AMOUNT_TEST + 1; i++) {
    for (int j = 0; j < AMOUNT_TRY; j++) {
      ...
      if(time.Splay.Insert == POISON_VAL_TEST) printf("мало памяти");
